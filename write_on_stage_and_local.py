...
class SnowflakeDataValidator:
    ...
    def write_mismatched_rows_to_csv(self, mismatched_rows: List[Dict], filename: str = None, write_local: bool = True):
        """
        Write mismatched rows to local CSV file
        ...
        try:
            ...
            self.logger.info(f"Mismatched rows written to local file: {filename}")
            
        except Exception as e:
            self.logger.error(f"Error writing mismatched rows to local CSV: {str(e)}")
    ...
    def write_validation_log_to_csv(self, structure_differences: List[Dict], 
                                  summary_stats: List[Dict], filename: str = None, write_local: bool = True):
        """
        Write validation log to local CSV file
        ...
        try:
            ...
            self.logger.info(f"Validation log written to local file: {filename}")
            
        except Exception as e:
            self.logger.error(f"Error writing validation log to local CSV: {str(e)}")
    ...
    def validate_tables(self, table1_name: str, table2_name: str, 
                       key_columns: List[str] = None, stage_name: str = "@~/", 
                       write_local_csv: bool = True) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            write_local_csv: Whether to also write local CSV files
        ...
        # Optionally write to local CSV files
        local_files = {}
        if write_local_csv:
            local_files['mismatched_rows'] = self.write_mismatched_rows_to_csv(mismatched_rows)
            local_files['validation_log'] = self.write_validation_log_to_csv(structure_differences, summary_stats)
        ...
        if write_local_csv:
            self.logger.info(f"Local CSV files: {local_files}")
...
def main():
    ...
    # Whether to write local CSV files (set to False if running in Snowflake stored procedure)
    ...
    try:
        ...
        if results:
            ...
            print(f"Snowflake stage: {results['stage_location']}")
            
            if write_local_csv and results['local_files']:
                print(f"Local CSV files:")
